...
dlf_names
```
But notice that instead of a list with an entry for each file that was read, we have a list with an entry for each log type that was read. All log of the same type are concatenated together, and an extra column "run" is added to the data field.
```{r}
name <- dlf_names[1]
head(dlfs[[name]]@data)
unique(dlfs[[name]]$run)
```
