...
head(sim[[names(sim)[1]]]@body$Date)
```

For the simulated data we will use the plot function `points_and_lines` from `daisyrVis`, which knows how to plot multiple `Dlf` objects together.
```{r}
points_and_lines(sim, "Date", "LAI")
```
...
```
This works because `points_and_lines` returns a `ggplot`object that we can add additional data and plots to. Note that this only works when you plot a single variable. If you use `point_and_lines` to plot multiple variables in different subplots, then you need to do things a bit differently.
...
field_dlfs <- list(
  NDVI=new("Dlf", header=list(info="Measured field data (NDVI sensor)"),
           ...
           body=data.frame(Date=ndvi$Date, LAI=ndvi$LAI_mean,
                           Height=rep(NA, nrow(ndvi)))),
  Plant=new("Dlf", header=list(info="Measured field data (Plant samples)"),
            units=data.frame(Date="", LAI="", Height="cm"),
            body=data.frame(Date=plant$Date, LAI=plant$LAI_mean,
                            Height=rep(NA, nrow(plant)))))
```

```{r}
points_and_lines(c(sim, field_dlfs), "Date", c("LAI", "Height"))
```
